...
using UnityEngine;
// Removed: using UnityEngine.Networking; // No longer needed if only WWW is used for these requests
...
public class ScoreManager : MonoBehaviour
{
	...
	public IEnumerator SubmitScore(string playerName, string email, int score)
	{
		...
		// --- Using WWW for POST request ---
		WWWForm form = new WWWForm();
		form.AddField("player_name", playerName);
		form.AddField("email", email);
		form.AddField("score", score.ToString()); // score needs to be string for AddField

		WWW submitWWW = new WWW(TopScoresURL, form); // Use WWW with form for POST

		yield return submitWWW; // Yield the WWW object directly

		if (submitWWW.error != null) // Check for errors using .error
		{
			Debug.LogError("ERROR SUBMITTING SCORE: " + submitWWW.error);
		}
		else
		{
			string responseText = submitWWW.text; // Get response text using .text
	...
	IEnumerator GetHighestScore()
	{
		...
		if (scoreList.Count > 0)
		{
			...
			if (scoreList[0].score != 1234 && scoreList[0].score != -123 && scoreList[0].name != "TIMEOUT ERROR" && scoreList[0].name != "DATABASE TEMPORARILY UNAVAILABLE")
			{
				...
				Debug.Log(string.Format("High score updated to: {0}, Lowest High updated to: {1}", _highscore, _lowestHigh));
			}
			else
			{
				Debug.LogWarning(string.Format("High score not updated due to an error entry as first score: {0}, {1}", scoreList[0].name, scoreList[0].score));
			}
		}
		else
		{
	...
	IEnumerator ReadScoresFromDB()
	{
		...
		// --- Using WWW for GET request ---
		WWW getScoresWWW = new WWW(TopScoresURL); // Use WWW for GET

		yield return getScoresWWW; // Yield the WWW object directly

		if (getScoresWWW.error != null) // Check for errors using .error
		{
			Debug.LogError("ERROR GETTING SCORES from DB: " + getScoresWWW.error);
		...
		{
			string rawResponse = getScoresWWW.text; // Get response text using .text
			...
			else
			{
				...
				for (int i = 0; i < names.Length; i++)
				{
					try
					{
						...
						Debug.Log(string.Format("Parsed Score in ReadScoresFromDB: Name='{0}', Value={1}", names[i], scores[i]));
					}
					catch (Exception e)
					{
